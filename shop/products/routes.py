...
from ..import db, app, photos, search
...
def deletebrand(id):
    ...
    if request.method=="POST":
        ...
        flash(f'The brand {brand.name} was deleted from your datebase', 'success')
        db.session.commit()
...
def deletecategory(id):
    ...
    if request.method=="POST":
        ...
        db.session.commit()
        flash(f'The category {category.name} was deleted from your datebase', 'success')
        return redirect(url_for('admin'))
...
def updateproduct(id):
    ...
    if request.method == 'POST':
        ...
        if request.files.get('image_1'):
            #Delete last image and upload new image from the folder
            try:
